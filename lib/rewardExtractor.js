...
function readValidators() {
  let validators = [];
  let data = fs.readFileSync(path.join(__dirname, '../data/solos_only.csv'), 'utf8');
...
const getWithdrawals = async (vindex, epoch, retryCount = 0) => {
  ...
  console.log("Extracting withdrawals for:");
  console.log(url);
  console.log(`\u{26CF} Extracting withdrawals for Epochs: ${epoch} - ${parseInt(epoch) + 100}`);

  try {
    const withdrawals = await rateLimiter.makeRequest(async () => {
    ...
    if (retryCount < maxRetries) {
      ...
      console.log(e);
      return await getWithdrawals(vindex, epoch, retryCount + 1);
    } else {
...
const getLatestEpoch = async () => {
  const rpcEndpoint = "http://libc-prod2:5052";

  console.log(`Fetching latest finalized epoch from ${rpcEndpoint}`);

  try {
    const response = await fetch(`${rpcEndpoint}/eth/v1/beacon/states/finalized/finality_checkpoints`, {
      method: 'GET',
      headers: {
    ...
    return { data: { epoch: finalizedEpoch } };
  }
  catch (e) {
    console.log("Error fetching latest epoch.");
    console.log(e);
    return { data: { epoch: 0 } }; // Return a default value or handle as appropriate
  }
}
...
const getEpochSlots = async (epoch) => {
  ...
  // Fetch the data adding the API key to the header.
  const slots = await rateLimiter.makeRequest(async () => {
    return await fetch(url, {
      headers: {
        apikey: API_KEY
      }
...
const getPerformance = async (vindex) => {
  let url = API_ENDPOINT + 'validator/' + vindex + '/performance';

  const performance = await rateLimiter.makeRequest(async () => {
    return await fetch(url, {
      headers: {
        apikey: API_KEY
      }
...
const getProposals = async (vindex, epoch) => {
  ...
  // A cool console log with reward emoji
  console.log(`\u{1F4B0} Extracting proposals for Epochs: ${epoch} - ${parseInt(epoch) + 100}`);

  // Fetch the data adding the API key to the header.
  const proposal = await rateLimiter.makeRequest(async () => {
    return await fetch(url, {
      headers: {
        apikey: API_KEY
      }
  ...
  for (let i = 0; i < validatorChunks.length; i++) {
    ...
    if (p.data?.length > 0) {
      for (let i = 0; i < p.data.length; i++) {
        ...
        const block = await rateLimiter.makeRequest(async () => {
          return await fetch(`https://beaconcha.in/api/v1/execution/block/${p.data[i].exec_block_number}`);
        });
        ...
        if (block_data.data?.length > 0 && block_data.data[0].relay) {
          console.log(`\u{1F6AA} Block mev relay found for block ${p.data[i].exec_block_number}. ${block_data.data[0].relay.tag}`);
          ...
          console.log(`\u{1F6AB} No MEV_RELAY found for block ${p.data[i].exec_block_number}.`);
          proposals.push({
            source: '',
...
const cleanRewards = async (rewards) => {
  ...
  // First lets get the list of validators.
  ...
  if (rewards.withdrawals.length > 0) {
    ...
    if (slots && slots.data) {
      ...
      console.log(`\u{1F6AB} Error getting slots for epoch: ${rewards.withdrawals[0].data[0].epoch}`);
    }
  }
  ...
  if (rewards.proposals.length > 0) {
    cleaned_proposals = await Promise.all(rewards.proposals.map(async (p) => {
      ...
      // const last_slot = slots.data[slots.data.length - 1];
      if (validatorsByIndex.hasOwnProperty(p.proposer)) {
        return {
        ...
        // Handle the case where the proposer is not found in validatorsByIndex
        // You might want to log this situation or handle it in some specific way
        console.warn(`Proposer not found in validatorsByIndex: ${p.proposer}`);
